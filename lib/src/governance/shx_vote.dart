...
/// no Soroban contract involved for voting. A ManageData entry is written
/// to the voting account with:
...
class ShxVote {
  ...
  final String votePositions; // e.g. "FOR" or "FOR,40" for multi-question proposals
...
class ShxGovernance {
  ...
  /// Per the reference implementation (vote.stronghold.co), pair this with
  /// [buildClearVoteOperation] in the same transaction to avoid an ongoing
  /// base reserve cost; clearing is optional and does not affect counting.
  static ManageDataOperationBuilder buildCastVoteOperation(ShxVote vote) {
    return ManageDataOperationBuilder(vote.dataEntryName, vote.votePositions);
  }
  ...
  // TODO(Phase 1): add a read helper that fetches an account's current vote
  // for a given proposal via Horizon's account data endpoint, decoding the
  // base64-encoded ManageData value back into vote positions.
}
